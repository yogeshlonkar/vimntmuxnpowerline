...
[ -f '/usr/libexec/java_home' ] && export JAVA_HOME=$(/usr/libexec/java_home) && export PATH = "$JAVA_HOME/bin:$PATH"
[ -f '/usr/local/Cellar/maven/3.6.0/' ] && export M2_HOME=/usr/local/Cellar/maven/3.6.0/libexec && export PATH = "$M2_HOME/bin:$PATH"
[ -f '/usr/local/opt/mongodb@3.6/' ] && export PATH="/usr/local/opt/mongodb@3.6/bin:$PATH"
[ -f '/usr/local/opt/node@8' ] && export PATH="/usr/local/opt/node@8/bin:$PATH"
if [ -f '~/Library/Python/2.7/' ]; then
	export PATH="~/Library/Python/2.7/bin:$PATH";
	export POWERLINE_PACKAGE_DIR=/usr/local/lib/python2.7/site-packages
fi
